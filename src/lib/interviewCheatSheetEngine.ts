...
const CACHE_KEY_PREFIX = 'sv_cheatsheet_enrichment_';

/** Cheap, non-cryptographic string hash — good enough to key an exact-match localStorage cache. */
...
function readCachedEnrichment(hash: string): CheatSheetEnrichment | null {
  try {
    ...
    if (!raw) return null;
    const parsed: CachedEnrichmentEntry = JSON.parse(raw);
    return parsed?.enrichment || null;
  } catch {
    return null;
  }
}
...
function writeCachedEnrichment(hash: string, enrichment: CheatSheetEnrichment): void {
  try {
    ...
    // localStorage może być niedostępny (tryb prywatny/limit) — cache to tylko opcjonalne usprawnienie.
  }
}

/**
 * Generates the Gemini-personalized part of the cheat sheet (POST /api/generate-cheat-sheet).
 * Checks an exact-match localStorage cache first so re-opening the same offer+vault
...
export async function generateCheatSheetEnrichmentWithAI(
  ...
  const response = await apiFetch('/api/generate-cheat-sheet', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ vault, targetRole, companyName, jobDescription, topRequirements }),
  });

  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(errorData.error || 'Nie udało się wygenerować spersonalizowanej ściągi przez Gemini Flash.');
  }

  const data = await response.json();
  const enrichment: CheatSheetEnrichment = data.enrichment;
